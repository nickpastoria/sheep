class Boxing extends Phaser.Scene {
    ...
    create () {
        ...
        this.boxer = this.add.sprite(game.config.width/2, 128, 'boxer',).setOrigin(0.5).play({key: this.boxer_tags[0].key, repeat: -1, yoyo: true, timeScale: 0.25});
        ...
        this.fistL.on(Phaser.Animations.Events.ANIMATION_UPDATE, (anim, frame, sprite, frameKey) => {
            if (frameKey == '3') {
                ...
                this.boxer.play({key: this.boxer_tags[1].key, timeScale:1.25});
            }
        });
        ...
        this.boxer.on(Phaser.Animations.Events.ANIMATION_COMPLETE, (anim, frame, sprite, frameKey) => {
            if (anim.key == this.boxer_tags[1].key) {
                this.boxer.play({key: this.boxer_tags[0].key, repeat: -1, yoyo: true, timeScale: 0.25})
            }
        })
    }
    ...
    update() {
        if (!this.transitioning) {
            if (Phaser.Input.Keyboard.JustDown(keySPACE)) {
                ...
                if (this.punchLeft) {
                    this.fistL.play({key:this.fist_tags[0].key, timeScale: .75 + this.magnitude/50});
                }
                else {
                    this.fistR.play({key:this.fist_tags[0].key, timeScale: .75 + this.magnitude/50});
                }
